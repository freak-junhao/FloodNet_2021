...
from models.senet.se_count import Senet_Count
...
def train(config, dataset):
    ...
    for epoch in range(start_epoch, config['MAX_EPOCH']):
        ...
        # Iteration
        model.train()
        for step, (image_data, num_gt, _) in enumerate(train_loader):
            optim.zero_grad()
...
def val(config, dataset):
    ...
    for step, (image_data, num_gt, _) in enumerate(dataloader):
...
def run_model():
    ...
    if args.run_mode == 'train':
        config['image_path'] += 'Train_Image_count_large/'
        config['question_path'] += 'Train_count_large.json'
        ...
        config['question_path'] += 'Test_count.json'
