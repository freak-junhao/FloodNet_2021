...
class CrossViT(nn.Module):
    ...
    def forward(self, img):
        ...
        # sm_cls, lg_cls = sm_tokens, lg_tokens
        ...
        return sm_logits + lg_logits, sm_y, lg_y


class Out_Line(nn.Module):
    def __init__(self, sm_in, lg_in, sm_dim, lg_dim, class_num):
        super().__init__()

        self.sm_conv = nn.Conv1d(sm_in, 1, 1)
        self.lg_conv = nn.Conv1d(lg_in, 1, 1)

        self.liner = nn.Sequential(
            nn.LayerNorm(sm_dim + lg_dim),
            nn.Linear(sm_dim + lg_dim, class_num)
        )
...
class Count_Cross_Vit(nn.Module):
    def __init__(self, config):
        super().__init__()

        self.config = config

        self.backbone = CrossViT(
            image_size=config['image_size'],
            num_classes=config['type_classes'],
            depth=config['depth'],
        ...
        sm_num = (config['image_size'] // config['sm_patch_size']) ** 2
        lg_num = (config['image_size'] // config['lg_patch_size']) ** 2
        self.output = Out_Line(sm_num, lg_num, config['sm_dim'], config['lg_dim'], config['num_classes'])

    def forward(self, data):
        x, y1, y2 = self.backbone(data)
        x = torch.sigmoid(x)
